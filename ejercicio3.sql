...
SELECT
    ...
    FORMAT_DATE('%Y%m%d', DATE(calls.start_date)) AS calls_start_date_id,
    calls.end_date AS calls_end_date,
    FORMAT_DATE('%Y%m%d', DATE(calls.end_date)) AS calls_end_date_id,
    ...
    steps.step_sequence AS step_sequece,
